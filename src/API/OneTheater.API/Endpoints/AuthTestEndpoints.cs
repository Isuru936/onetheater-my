...
internal sealed class AuthTestEndpoints : IEndpoint
{
    public void MapEndpoint(IEndpointRouteBuilder app)
    {
        ...
        authGroup.MapGet("admin-only", [Authorize(Policy = "AdminOnly")] (ClaimsPrincipal user) => new
        {
            Message = "This endpoint requires the 'admin' realm role",
            Username = user.Identity?.Name ?? "Unknown",
            UserId = user.FindFirst("sub")?.Value,
            RealmRoles = user.FindAll("roles").Select(c => c.Value).ToList(),
            Timestamp = DateTime.UtcNow,
            Status = "success"
        })
        ...
        authGroup.MapGet("client-or-admin", [Authorize(Policy = "ClientOrAdmin")] (ClaimsPrincipal user) => new
        {
        ...
        // User info endpoint
        authGroup.MapGet("user-info", [Authorize] (ClaimsPrincipal user, HttpContext context) => new
        {
